...
export const timeline = register(
  ...
  (idPat, pat) => {
    ...
    const data = idPat.withHap((hap) => {
      ...
      const p = id > 0 ? 1 : -1; // lowercase is current polarity / time, upper is cached
      const t = Number(hap.whole.begin);
      const P = polarities[key] ?? p;
      const T = state[key] ?? t; // align the pattern with the next cycle
      hap.value = {
    ...
    const times = data.withValue((v) => v.offset);
    return pat
      .late(times)
      .withValue((v) => (d) => ({ ...v, timeline: d.id, offset: d.offset }))
      .appLeft(data)
      .onTrigger((hap) => {
        const { timeline: id, offset } = hap.value;
        const key = Math.abs(id);
