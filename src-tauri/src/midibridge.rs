...
pub fn init(
    ...
    tauri::async_runtime::spawn(async move {
        ...
        if out_ports.len() == 0 {
            log::warn!(
                " No MIDI devices found. Connect a device or enable IAC Driver to enable midi."
            );
        ...
        log::info!("Found {} midi devices!", out_ports.len());
            ...
            message_queue.retain(|message| {
                ...
                if out_con.is_some() {
                    ...
                    if let Err(err) = (&mut out_con.unwrap()).send(&message.message) {
                        log::error!("Midi message send error: {}", err);
                    }
                } else {
                    log::error!("failed to find midi device: {}", message.requestedport);
                }
                return false;
            });
